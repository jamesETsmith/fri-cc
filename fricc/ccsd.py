...
def update_amps(mycc, t1, t2, eris):
    ...
    t2new *= 0.5  # *.5 because t2+t2.transpose(1,0,3,2) in the end
    ...
    foo += 0.5 * numpy.einsum("ia,ja->ij", fock[:nocc, nocc:], t1)

    fvv = fock[nocc:, nocc:] - numpy.diag(mo_e_v)
    fvv -= 0.5 * numpy.einsum("ia,ib->ab", t1, fock[:nocc, nocc:])
    ...
    blksize = min(nvir, max(BLKMIN, int((max_memory * 0.9e6 / 8 - nocc ** 4) / unit)))
    log.debug1(
        "max_memory %d MB,  nocc,nvir = %d,%d  blksize = %d",
        max_memory,
        nocc,
        nvir,
        blksize,
    ...
    for p0, p1 in lib.prange(0, nvir, blksize):
        ...
        tau = numpy.einsum("ia,jb->ijab", t1[:, p0:p1] * 0.5, t1)
        tau += t2[:, :, p0:p1]
        ...
        with lib.call_in_background(
            update_wVooV, sync=not mycc.async_io
        ) as update_wVooV:
        ...
        wVOov += eris_voov
        eris_VOov = -0.5 * eris_voov.transpose(0, 2, 1, 3)
        eris_VOov += eris_voov
        eris_voov = None

        def update_wVOov(q0, q1, tau):
            wVOov[:, :, :, q0:q1] += 0.5 * lib.einsum("aikc,kcjb->aijb", eris_VOov, tau)
    ...
    for p0, p1 in lib.prange(0, nvir, blksize):
        ...
        tau += t2[:, :, p0:p1]
        t2new[:, :, p0:p1] += 0.5 * lib.einsum("ijkl,klab->ijab", woooo, tau)
        theta = tau = None

    ft_ij = foo + numpy.einsum("ja,ia->ij", 0.5 * t1, fov)
    ft_ab = fvv - numpy.einsum("ia,ib->ab", 0.5 * t1, fov)
...
def _add_ovvv_(mycc, t1, t2, eris, fvv, t1new, t2new, fswap):
    ...
    blksize = min(nvir, max(BLKMIN, int((max_memory * 0.95e6 / 8 - wooVV.size) / unit)))
    if not mycc.direct:
        unit = (
            nocc * nvir ** 2 * 3 + nocc ** 2 * nvir + 2 + nocc * nvir ** 2 + nocc * nvir
        )
        blksize = min(
            nvir,
            max(
                BLKMIN,
                int((max_memory * 0.95e6 / 8 - wooVV.size - nocc ** 2 * nvir) / unit),
            ),
        )
    log.debug1(
        "max_memory %d MB,  nocc,nvir = %d,%d  blksize = %d",
    ...
    with lib.call_in_background(load_ovvv, sync=not mycc.async_io) as prefetch:
        ...
        for p0, p1 in lib.prange(0, nvir, blksize):
            ...
            lib.ddot(
                numpy.asarray(t1[:, p0:p1], order="C"),
                eris_vovv.reshape(p1 - p0, -1),
                -1,
                wooVV,
                1,
            )
...
def _add_vvvv_tril(mycc, t1, t2, eris, out=None, with_ovvv=None):
    ...
    if mycc.direct:  # AO-direct CCSD
        ...
        tau = _ao2mo.nr_e2(
            tau.reshape(nocc2, nvir ** 2), aos, (0, nao, 0, nao), "s1", "s1"
        )
        ...
        if with_ovvv:
            ...
            Ht2tril -= lib.ddot(tmp.reshape(nocc2 * nvir, nocc), t1).reshape(
                nocc2, nvir, nvir
            )
            ...
            tmp = lib.ddot(
                tmp.reshape(nocc2 * nvir, nocc),
                t1,
                1,
                numpy.ndarray((nocc2 * nvir, nvir), buffer=tau),
                0,
            )
...
def _add_vvvv_full(mycc, t1, t2, eris, out=None, with_ovvv=False):
    ...
    if mycc.direct:  # AO-direct CCSD
        ...
        aos = numpy.asarray(mo[:, nocc:].T, order="F")
        tau = _ao2mo.nr_e2(
            tau.reshape(nocc ** 2, nvir, nvir), aos, (0, nao, 0, nao), "s1", "s1"
        )
...
def _contract_s4vvvv_t2(mycc, mol, vvvv, t2, out=None, verbose=None):
    ...
    if vvvv is None:  # AO-direct CCSD
        ...
        ao2mopt = _ao2mo.AO2MOpt(
            mol, intor, "CVHFnr_schwarz_cond", "CVHFsetnr_direct_scf"
        )
        blksize = max(BLKMIN, numpy.sqrt(max_memory * 0.9e6 / 8 / nvirb ** 2 / 2.5))
        ...
        for ip, (ish0, ish1, ni) in enumerate(sh_ranges):
            for jsh0, jsh1, nj in sh_ranges[:ip]:
                ...
                time0 = log.timer_debug1(
                    "AO-vvvv [%d:%d,%d:%d]" % (ish0, ish1, jsh0, jsh1), *time0
                )
            ...
            time0 = log.timer_debug1(
                "AO-vvvv [%d:%d,%d:%d]" % (ish0, ish1, ish0, ish1), *time0
            )
        ...
        blksize = numpy.sqrt(max(BLKMIN ** 2, max_memory * 0.95e6 / 8 / unit))
        blksize = int(min((nvira + 3) / 4, blksize))
...
def _unpack_4fold(c2vec, nocc, nvir, anti_symm=True):
    ...
    if nocc > 1 and nvir > 1:
        ...
        lib.takebak_2d(
            t2, t2tril, otril[0] * nocc + otril[1], vtril[0] * nvir + vtril[1]
        )
        lib.takebak_2d(
            t2, t2tril, otril[1] * nocc + otril[0], vtril[1] * nvir + vtril[0]
        )
        if anti_symm:  # anti-symmetry when exchanging two particle indices
            t2tril = -t2tril
        lib.takebak_2d(
            t2, t2tril, otril[0] * nocc + otril[1], vtril[1] * nvir + vtril[0]
        )
        lib.takebak_2d(
            t2, t2tril, otril[1] * nocc + otril[0], vtril[0] * nvir + vtril[1]
        )
...
def amplitudes_to_vector_s4(t1, t2, out=None):
    ...
    lib.take_2d(
        ...
        otril[0] * nocc + otril[1],
        vtril[0] * nvir + vtril[1],
        out=vector[nov:],
    )
    return vector
...
def energy(mycc, t1=None, t2=None, eris=None):
    ...
    max_memory = mycc.max_memory - lib.current_memory()[0]
    blksize = int(
        min(nvir, max(BLKMIN, max_memory * 0.3e6 / 8 / (nocc ** 2 * nvir + 1)))
    )
    for p0, p1 in lib.prange(0, nvir, blksize):
        eris_ovvo = eris.ovvo[:, p0:p1]
        tau = t2[:, :, p0:p1] + numpy.einsum("ia,jb->ijab", t1[:, p0:p1], t1)
...
class CCSD(lib.StreamObject):
    ...
    diis_start_energy_diff = getattr(
        __config__, "cc_ccsd_CCSD_diis_start_energy_diff", 1e9
    )
...
http://sunqm.net/pyscf/code-rule.html#api-rules for the details of API conventions"""
    ...
    def dump_flags(self, verbose=None):
        ...
        if log.verbose >= logger.DEBUG1 and self.__class__ == CCSD:
            nocc = self.nocc
    ...
    def init_amps(self, eris=None):
        ...
        for p0, p1 in lib.prange(0, nvir, blksize):
            ...
            t2[:, :, p0:p1] = eris_ovov.transpose(0, 2, 1, 3).conj() / lib.direct_sum(
                "ia,jb->ijab", eia[:, p0:p1], eia
            )
        ...
        logger.info(
            self,
            "Init t2, MP2 energy = %.15g  E_corr(MP2) %.15g",
            e_hf + self.emp2,
            self.emp2,
        )
    ...
    def solve_lambda(self, t1=None, t2=None, l1=None, l2=None, eris=None):
        ...
        if eris is None:
            eris = self.ao2mo(self.mo_coeff)
        self.converged_lambda, self.l1, self.l2 = ccsd_lambda.kernel(
            self,
            eris,
            t1,
            t2,
    ...
    def ipccsd(
        ...
        return eom_rccsd.EOMIP(self).kernel(
            nroots, left, koopmans, guess, partition, eris
        )
    ...
    def eaccsd(
        ...
        return eom_rccsd.EOMEA(self).kernel(
            nroots, left, koopmans, guess, partition, eris
        )
    ...
    def ao2mo(self, mo_coeff=None):
        ...
        if self._scf._eri is not None and (
            ...
            logger.warn(
                self,
                "CCSD detected DF being used in the HF object. "
    ...
    def run_diis(self, t1, t2, istep, normt, de, adiis):
        if (
            adiis
            and istep >= self.diis_start_cycle
    ...
    def dump_chk(self, t1_t2=None, frozen=None, mo_coeff=None, mo_occ=None):
        ...
        cc_chk = {"e_corr": self.e_corr, "t1": t1, "t2": t2, "frozen": frozen}

        if mo_coeff is not None:
            cc_chk["mo_coeff"] = mo_coeff
        if mo_occ is not None:
            cc_chk["mo_occ"] = mo_occ
        if self._nmo is not None:
            cc_chk["_nmo"] = self._nmo
        if self._nocc is not None:
            cc_chk["_nocc"] = self._nocc
...
def _make_eris_outcore(mycc, mo_coeff=None):
    ...
    eris.ovov = eris.feri1.create_dataset(
    ...
    def save_vir_frac(p0, p1, eri):
        ...
        vvv = lib.pack_tril(
            eri[:, :, nocc:, nocc:].reshape((p1 - p0) * nocc, nvir, nvir)
        )
    ...
    ao2mo.outcore.half_e1(
        mol, (mo_coeff, orbo), fswap, int2e, "s4", 1, max_memory, verbose=log
    )

    ao_loc = mol.ao_loc_nr()
    nao_pair = nao * (nao + 1) // 2
    blksize = int(min(8e9, max_memory * 0.5e6) / 8 / (nao_pair + nmo ** 2) / nocc)
...
def _make_df_eris_outcore(mycc, mo_coeff=None):
    ...
    for eri1 in mycc._scf.with_df.loop():
        Lpq = _ao2mo.nr_e2(eri1, mo_coeff, ijslice, aosym="s2", out=Lpq).reshape(
            -1, nmo, nmo
        )
...
def _flops(nocc, nvir):
    ...
    return (
        nocc ** 3 * nvir ** 2 * 2
        + nocc ** 2 * nvir ** 3 * 2
        + nocc ** 4 * nvir * 2 * 2  # Ftilde
        + nocc ** 4 * nvir ** 2 * 2
        + nocc * nvir ** 4 * 2 * 2  # Wijkl
        + nocc ** 2 * nvir ** 3 * 2  # Wabcd
        + nocc ** 3 * nvir ** 2 * 2
        + nocc ** 3 * nvir ** 3 * 2
        + nocc ** 3 * nvir ** 3 * 2
        + nocc ** 2 * nvir ** 3 * 2
        + nocc ** 3 * nvir ** 2 * 2
        + nocc ** 2 * nvir ** 3 * 2  # Wiabj
        + nocc ** 3 * nvir ** 2 * 2
        + nocc ** 3 * nvir ** 2 * 2 * 2  # t1
        + nocc ** 4 * nvir ** 2 * 2
        + nocc * (nocc + 1) / 2 * nvir ** 4 * 2
        + nocc ** 2 * nvir ** 3 * 2 * 2  # vvvv
        + nocc ** 3 * nvir ** 2 * 2 * 2
        + nocc ** 3 * nvir ** 3 * 2  # t2
        + nocc ** 3 * nvir ** 3 * 2 * 2
        + nocc ** 3 * nvir ** 2 * 2 * 4
